...
def run_clustering(df: pd.DataFrame, n_clusters: int = 4, bpm: Optional[float] = None, auto_k: bool = False) -> Dict[str, Any]:
    """
    ...
    if USE_IMPROVED_ALGORITHM:
        try:
            ...
            # Generate cluster info with names/colors
            cluster_info = []
            cluster_colors = ["#EAE2B7", "#FCBF49", "#F77F00", "#D62828", "#003049", "#9B59B6", "#2ECC71"]
            cluster_names = ["Chill Flow", "Focus Pulse", "Energy Rush", "Intense Beats", "Deep Groove", "Ethereal", "Dynamic"]
            cluster_tags = [
                ["lo-fi", "calm"],
...
def cluster_tracks():
    ...
    try:
        ...
        cached_bpm = float(bpm)

        log.info("Running clustering for BPM: %s, clusters: %d", bpm, n_clusters)
        ...
        # Run clustering
        result = run_clustering(df, n_clusters=n_clusters, bpm=bpm)
